...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <div className={`flex gap-4 items-start ${isBot ? "" : "flex-row-reverse"}`}>
      ...
      >
        {isBot ? (
          <img src={chatIconImage} alt="TuneTalk" className="w-10 h-10 object-contain" />
        ) : (
      ...
      <div className={`flex-1 ${isBot ? "" : "flex flex-col items-end"}`}>
          ...
          >
            ...
            {message.emotion && (
              <div className="mt-2 flex items-center gap-1.5 text-xs opacity-90">
                ...
                <span className="font-medium capitalize">{message.emotion.label}</span>
                {message.emotion.confidence && (
                  <span className="opacity-75">({Math.round(message.emotion.confidence * 100)}%)</span>
                )}
              </div>
            )}
          </div>
        ...
        <p className={`text-xs mt-2 ${isBot ? "text-gray-500" : "text-gray-500"}`}>
